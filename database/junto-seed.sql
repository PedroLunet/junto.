...
--
DO $do$
DECLARE s text := COALESCE(current_setting('app.schema', true), 'lbaw2544');
BEGIN -- identifiers require dynamic SQL
...
CREATE FUNCTION users_search_update() RETURNS trigger AS $$ BEGIN IF TG_OP = 'INSERT' THEN NEW.fts_document = (
    setweight(
...
CREATE TRIGGER users_search_update_trigger BEFORE
INSERT
    OR
UPDATE ON users FOR EACH ROW EXECUTE PROCEDURE users_search_update();
...
CREATE FUNCTION groups_search_update() RETURNS trigger AS $$ BEGIN IF TG_OP = 'INSERT' THEN NEW.fts_document = to_tsvector('english', coalesce(NEW.name, ''));
END IF;
IF TG_OP = 'UPDATE' THEN IF (NEW.name <> OLD.name) THEN NEW.fts_document = to_tsvector('english', coalesce(NEW.name, ''));
END IF;
END IF;
RETURN NEW;
END $$ LANGUAGE plpgsql;
-- 3. Create the trigger
CREATE TRIGGER groups_search_update_trigger BEFORE
INSERT
    OR
UPDATE ON groups FOR EACH ROW EXECUTE PROCEDURE groups_search_update();
-- 4. Create the GIN index
...
CREATE FUNCTION standard_post_search_update() RETURNS trigger AS $$ BEGIN IF TG_OP = 'INSERT' THEN NEW.fts_document = to_tsvector('english', coalesce(NEW.text, ''));
END IF;
IF TG_OP = 'UPDATE' THEN IF (NEW.text <> OLD.text) THEN NEW.fts_document = to_tsvector('english', coalesce(NEW.text, ''));
END IF;
END IF;
RETURN NEW;
END $$ LANGUAGE plpgsql;
-- 3. Create the trigger
CREATE TRIGGER standard_post_search_update_trigger BEFORE
INSERT
    OR
UPDATE ON standard_post FOR EACH ROW EXECUTE PROCEDURE standard_post_search_update();
-- 4. Create the GIN index
...
CREATE FUNCTION review_search_update() RETURNS trigger AS $$ BEGIN IF TG_OP = 'INSERT' THEN NEW.fts_document = to_tsvector('english', coalesce(NEW.content, ''));
END IF;
IF TG_OP = 'UPDATE' THEN IF (NEW.content <> OLD.content) THEN NEW.fts_document = to_tsvector('english', coalesce(NEW.content, ''));
END IF;
END IF;
RETURN NEW;
END $$ LANGUAGE plpgsql;
-- 3. Create the trigger
CREATE TRIGGER review_search_update_trigger BEFORE
INSERT
    OR
UPDATE ON review FOR EACH ROW EXECUTE PROCEDURE review_search_update();
-- 4. Create the GIN index
...
AFTER
INSERT ON post_like FOR EACH ROW EXECUTE FUNCTION fn_notify_post_like();
...
AFTER
INSERT ON comment_like FOR EACH ROW EXECUTE FUNCTION fn_notify_comment_like();
...
AFTER
INSERT ON comment FOR EACH ROW EXECUTE FUNCTION fn_notify_comment();
...
CREATE OR REPLACE FUNCTION fn_notify_friend_request() RETURNS TRIGGER AS $$ BEGIN -- No action needed - notification already created by fn_send_friend_request()
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
AFTER
INSERT ON friend_request FOR EACH ROW EXECUTE FUNCTION fn_notify_friend_request();
-- FUNCTION: Notify Group Join Request
CREATE OR REPLACE FUNCTION fn_notify_group_join_request() RETURNS TRIGGER AS $$ BEGIN
INSERT INTO notification (message, receiverId)
SELECT CONCAT(
        'User ',
        (
            ...
            WHERE id = (
                    SELECT senderId
                    FROM request
                    WHERE notificationId = NEW.requestId
                )
        ),
...
AFTER
INSERT ON group_join_request FOR EACH ROW EXECUTE FUNCTION fn_notify_group_join_request();
-- FUNCTION: Anonymize User Data
CREATE OR REPLACE FUNCTION fn_anonymize_user_data(p_user_id INT) RETURNS VOID AS $$ BEGIN
UPDATE users
...
WHERE userId1 = p_user_id
    OR userId2 = p_user_id;
...
$$ LANGUAGE plpgsql;
-- US24 - Report Content
CREATE OR REPLACE FUNCTION fn_report_post(p_post_id INT, p_reason TEXT) RETURNS VOID AS $$ BEGIN
SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
...
CREATE OR REPLACE FUNCTION fn_delete_account(p_user_id INT) RETURNS VOID AS $$ BEGIN
...
$$ LANGUAGE plpgsql;
-- US29 - Unfriend
CREATE OR REPLACE FUNCTION fn_unfriend(p_user1_id INT, p_user2_id INT) RETURNS VOID AS $$ BEGIN
SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
-- Simple delete operation; stronger isolation not needed.
DELETE FROM friendship
WHERE (
        userId1 = p_user1_id
        AND userId2 = p_user2_id
    )
    OR (
        userId1 = p_user2_id
        AND userId2 = p_user1_id
    );
END;
$$ LANGUAGE plpgsql;
-- US30 - Comment on Post
CREATE OR REPLACE FUNCTION fn_comment_on_post(p_post_id INT, p_user_id INT, p_content TEXT) RETURNS VOID AS $$ BEGIN
SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
...
$$ LANGUAGE plpgsql;
-- US31 - React to Post
CREATE OR REPLACE FUNCTION fn_react_to_post(p_post_id INT, p_user_id INT) RETURNS VOID AS $$ BEGIN
SET TRANSACTION ISOLATION LEVEL REPEATABLE READ;
...
CREATE OR REPLACE FUNCTION fn_react_to_comment(p_comment_id INT, p_user_id INT) RETURNS VOID AS $$ BEGIN
SET TRANSACTION ISOLATION LEVEL REPEATABLE READ;
...
VALUES (
        1,
        'Just watched Inception again. Still brilliant.',
        'inception-post.jpg'
    ),
    (
        2,
        'Reading The Great Gatsby this weekend.',
        NULL
    ),
    (
        5,
...
VALUES (
    ...
    (
        'Bookworms',
        'Share and review your favorite books',
        TRUE,
        'bookworms.jpg'
    ),
    (
        'Music Lovers',
        'Everything about records and concerts',
        FALSE,
        'music-lovers.jpg'
    );
INSERT INTO membership (userId, groupId, isOwner)

...
<div class="container mx-auto px-4 py-8">
        ...
        <div class="space-y-4">
            @foreach($sentRequests as $sentRequest)
                ...
                <div class="bg-white shadow rounded-lg p-6 flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <div class="w-12 h-12 rounded-full bg-purple-200 flex items-center justify-center">
                            @if($receiver->profilepicture)
                                <img src="{{ asset($receiver->profilepicture) }}" alt="{{ $receiver->name }}" class="w-12 h-12 rounded-full object-cover">
                            @else
                    ...
                    <div>
                        <form action="{{ route('friend-requests.cancel', $sentRequest->requestid) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
    ...
    <div class="mt-6">
        <a href="{{ route('friend-requests.index') }}" class="text-purple-600 hover:text-purple-800">← Back to received requests</a>
    </div>
</div>
@endsection

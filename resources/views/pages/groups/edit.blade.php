            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div class="p-8">
                    <form method="POST" action="{{ route('groups.update', $group->id) }}">
                        ...
                        <x-ui.input label="Group Name" name="name" type="text" value="{{ old('name', $group->name) }}"
                        ...
                        <x-ui.input label="Description" name="description" type="textarea"
                            value="{{ old('description', $group->description) }}" placeholder="What is this group about?"
                            :error="$errors->first('description')" rows="4" class="text-xl" />
                            ...
                            <label class="flex items-start cursor-pointer">
                                <div class="flex items-center h-5">
                                    <input type="checkbox" name="isPrivate" id="isPrivate" value="1"
                                        {{ old('isPrivate', $group->isprivate) ? 'checked' : '' }}

                    ...
                    <div class="px-6 pb-6 relative">
                        <div class="-mt-16 mb-4">
                            <div class="bg-[#820263]/40 p-2 rounded-xl shadow-sm inline-block">
                                <div
                                    class="h-24 w-24 bg-[#820263] rounded-lg flex items-center justify-center text-white text-4xl font-extrabold shadow-inner">
                                    {{ substr($group->name, 0, 1) }}
                                </div>
                            </div>
                        </div>

                        <h2 class="text-3xl font-black text-gray-900 mb-3 tracking-tight leading-tight">
                            {{ $group->name }}</h2>

                        <div class="mb-4">
                            @if ($group->isprivate)
                                <span
                                    class="inline-flex items-center bg-amber-50 text-amber-700 text-sm px-3 py-1 rounded-full font-bold border border-amber-200 shadow-sm">
                                    <i class="fas fa-lock mr-1.5"></i> Private
                                </span>
                            @else
                                <span
                                    class="inline-flex items-center bg-green-50 text-green-700 text-sm px-3 py-1 rounded-full font-bold border border-green-200 shadow-sm">
                        ...
                        <div
                            class="flex items-center justify-center gap-8 text-gray-500 border-t border-gray-100 pt-6 mb-6">
                            <div class="text-center">
                                <span
                                    class="block font-black text-gray-900 text-2xl mb-1">{{ $group->users_count ?? 0 }}</span>
                            ...
                            <div class="flex flex-col gap-3 pt-2">
                                @if ($isOwner)
                                    <x-ui.button href="{{ route('groups.edit', $group) }}" variant="secondary"
                                        class="w-full justify-center shadow-sm border border-gray-200 py-3 text-base font-bold">
                                ...
                                @if ($group->members->contains(auth()->user()))
                                    <form action="{{ route('groups.leave', $group) }}" method="POST" class="w-full">
                                        ...
                                        <x-ui.button type="submit" variant="danger"
                                            class="w-full justify-center shadow-sm py-3 text-base font-bold">
                                ...
                                @elseif(isset($pendingRequest) && $pendingRequest)
                                    <form action="{{ route('groups.cancelRequest', $group) }}" method="POST"
                                        ...
                                        <x-ui.button type="submit" variant="secondary"
                                            class="w-full justify-center bg-gray-100 text-gray-700 hover:bg-gray-200 py-3 text-base font-bold">
                                ...
                                @else
                                    <form action="{{ route('groups.join', $group) }}" method="POST" class="w-full">
                                        @csrf
                                        <x-ui.button type="submit" variant="primary"
                                            class="w-full justify-center shadow-md bg-[#820263] hover:bg-[#600149] py-3 text-base font-bold">
                        ...
                        @else
                            <x-ui.button href="{{ route('login') }}" variant="primary"
                                class="w-full justify-center shadow-md bg-[#820263] hover:bg-[#600149] mt-2 py-3 text-base font-bold">
                                Log in to Join
                            </x-ui.button>
                        @endif
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                    ...
                    @php
                        ...
                        if (
                            $isOwnerView ||
                            !$group->isprivate ||
                            (auth()->check() && $group->members->contains(auth()->user()))
                        ) {
                            $members = $group->members;
                        } elseif (auth()->check()) {
                            $members = $group->members->intersect(auth()->user()->friends());
                        } else {
                            $members = collect();
                        }

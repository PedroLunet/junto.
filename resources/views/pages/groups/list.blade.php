...
@section('content')
    <div class="container mx-auto px-4 py-12">
        ...
        <div class="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            <div>
                <h1 class="text-6xl font-black text-gray-900 tracking-tight">Groups</h1>
                <p class="text-gray-500 mt-4 text-2xl font-normal">Discover communities and join the conversation.</p>
            </div>
            <x-ui.button href="{{ route('groups.create') }}" variant="primary" class="shadow-md bg-[#820263] hover:bg-[#600149] py-5 px-10 text-xl font-bold">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            @forelse ($groups as $group)
                <a href="{{ route('groups.show', $group) }}" class="flex flex-col bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-xl hover:-translate-y-2 transition-all duration-300 group cursor-pointer block">
                    ...
                    <div class="px-8 pb-10 flex-1 flex flex-col relative">

                        <div class="-mt-20 mb-8">
                            <div class="bg-white p-3 rounded-[2rem] shadow-sm inline-block">
                                <div class="h-32 w-32 bg-[#820263] rounded-[1.5rem] flex items-center justify-center text-white text-6xl font-extrabold shadow-inner">
                        ...
                        <div class="mb-6">
                            <h2 class="text-4xl font-black text-gray-900 leading-tight mb-4 group-hover:text-[#820263] transition-colors">
                                {{ $group->name }}
                            </h2>
                            
                            <div class="flex items-center gap-3">
                                @if($group->isprivate)
                                    <span class="inline-flex items-center bg-amber-50 text-amber-700 text-base px-4 py-1.5 rounded-full font-bold border border-amber-200">
                                        <i class="fas fa-lock mr-2"></i> Private
                                    </span>
                                @else
                                    <span class="inline-flex items-center bg-green-50 text-green-700 text-base px-4 py-1.5 rounded-full font-bold border border-green-200">
                        ...
                        <p class="text-gray-700 mb-10 text-lg leading-relaxed line-clamp-3 flex-1">
                            {{ $group->description }}
                        </p>

                        <div class="pt-8 border-t border-gray-100 flex items-center justify-between">
                            <div class="flex items-center text-gray-500 text-lg font-medium">
                                <i class="fas fa-users mr-3 text-gray-400"></i>
                                {{ $group->users_count ?? 0 }} Members
                            </div>
                            
                            <span class="text-[#820263] font-bold text-lg group-hover:underline flex items-center">
                                View Group <i class="fas fa-arrow-right ml-3 text-base transition-transform group-hover:translate-x-1"></i>
                            </span>
                        </div>
                    </div>
                </a>

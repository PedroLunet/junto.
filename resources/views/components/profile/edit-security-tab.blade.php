<div class="space-y-8">
    <div>
        <h2 class="text-xl sm:text-2xl font-semibold text-gray-900 py-4 mb-6 border-b border-[#7a5466]">Account Settings
        </h2>
    ...
    <div>
        ...
        <div
            class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between py-4 border-b border-gray-200 gap-2 sm:gap-0">
            <div>
                <h3 class="text-lg sm:text-xl font-medium text-gray-900">Make account private</h3>
                <p class="text-sm sm:text-base text-gray-500 mt-1">Only your friends can see your posts.</p>
            </div>
        ...
        <div
            ...
            <x-ui.button href="{{ url('/logout') }}" type="button" variant="secondary"
                class="text-base text-gray-700 whitespace-nowrap w-full sm:w-auto flex justify-center text-center">
                Log out
            </x-ui.button>
        </div>

        <!-- Delete my account -->
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between py-4 gap-2 sm:gap-0">
            <div>
                <h3 class="text-lg sm:text-xl font-medium text-red-700">Delete my account</h3>
                <p class="text-sm sm:text-base text-gray-500 mt-1">Permanently delete the account.</p>
            </div>
            <x-ui.button type="button" variant="danger" class="text-base whitespace-nowrap w-full sm:w-auto"
    ...
    <div class="bg-white rounded-lg p-6 max-w-sm w-full mx-4">
        ...
        <div class="flex gap-4">
            <button type="button" onclick="closeDeleteAccountModal()"
                class="flex-1 px-4 py-2 bg-gray-300 text-gray-900 rounded hover:bg-gray-400">Cancel</button>
            <button type="button" onclick="confirmDeleteAccount()"
                class="flex-1 px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Delete</button>
        </div>
    </div>
</div>
...
<script>
    ...
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Escape' && !document.getElementById('deleteAccountModal').classList.contains(
                'hidden')) {
            closeDeleteAccountModal();
        }
    });

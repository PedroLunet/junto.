<div class="relative shrink-0 flex gap-0 border-b border-gray-200 mb-4 sm:mb-6 px-2 sm:px-8 lg:px-10" id="tabs-bar">
    ...
    @endforeach
    <span id="tab-underline" class="absolute bottom-0 h-1 bg-[#820273] rounded transition-all duration-300" style="left:0;width:0;"></span>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    const tabKeys = [@foreach ($tabs as $key => $tab)'{{ $key }}'@if(!$loop->last),@endif @endforeach];
    ...
    window.addEventListener('resize', function() {
        const activeTab = tabKeys.map(k => document.getElementById(k+'-tab')).find(tab => tab.classList.contains('text-[#820273]'));

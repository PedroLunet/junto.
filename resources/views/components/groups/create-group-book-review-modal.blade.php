<div id="create-group-book-review-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50" style="display: none;">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-2xl shadow-xl max-w-6xl w-full min-h-[400px]">
            ...
            <div class="p-8">
                <form id="create-group-book-review-form"
                    action="{{ route('groups.reviews.store', ['group' => $group->id, 'type' => 'book']) }}"
                    method="POST">
                    @csrf
                    <input type="hidden" name="type" value="book">
                    <input type="hidden" name="group_id" value="{{ $group->id }}">

                    <x-ui.media-search type="book" searchId="groupModalBookSearch"
                        searchResultsId="groupModalBookSearchResults" selectedId="groupModalSelectedBook"
                        inputName="google_book_id" removeBtnId="groupRemoveBookBtn"
                        searchPlaceholder="Search for a book..." label="What book did you read?" />
                    ...
                    <div class="flex justify-end space-x-3">
                        <x-ui.button type="button" id="cancel-group-book-review-button"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const searchContainer = document.getElementById('groupModalBookSearchContainer');
                ...
                timeoutId = setTimeout(() => {
                    fetch(`/books/search?q=${encodeURIComponent(query)}`, {
                        ...
                        .then(books => {
                            ...
                            if (books.length === 0) {
                                resultsDiv.innerHTML =
                                    '<div class="p-3 text-gray-500">No books found</div>';
                            } else {
                                books.slice(0, 5).forEach(book => {
                                    ...
                                    const safeTitle = (book.title || '').replace(
                                        /'/g, "&apos;");
                                    const safeAuthor = (book.creator || '').replace(
                                        /'/g, "&apos;");
                            ...
                            `;
                                    ...
                                    div.addEventListener('click', () => {
                                        ...
                                        selectedTitle.textContent = book
                                            .title;
                                        selectedAuthor.textContent = book
                                            .creator;
                                        selectedYear.textContent = book
                                            .releaseyear || '';
                                        selectedCover.src = book
                                            .coverimage || '';

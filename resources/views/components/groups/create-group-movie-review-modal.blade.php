<div id="create-group-movie-review-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50" style="display: none;">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-2xl shadow-xl max-w-6xl w-full min-h-[400px]">
            ...
            <div class="p-8">
                <form id="create-group-movie-review-form"
                    action="{{ route('groups.reviews.store', ['group' => $group->id, 'type' => 'movie']) }}"
                    method="POST">
                    @csrf
                    <input type="hidden" name="type" value="movie">
                    <input type="hidden" name="group_id" value="{{ $group->id }}">

                    <x-ui.media-search type="movie" searchId="groupModalMovieSearch"
                        searchResultsId="groupModalMovieSearchResults" selectedId="groupModalSelectedMovie"
                        inputName="tmdb_id" removeBtnId="groupRemoveMovieBtn" searchPlaceholder="Search for a movie..."
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex gap-2" id="group-movie-star-rating">
                            @for ($i = 1; $i <= 5; $i++)
                                <button type="button"
                                    class="group-movie-star-btn text-3xl text-gray-300 hover:text-yellow-400 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:rounded-full transition-colors transform active:scale-95"
                                    data-rating="{{ $i }}">
                    ...
                    <div class="flex justify-end space-x-3">
                        <x-ui.button type="button" id="cancel-group-movie-review-button"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const searchContainer = document.getElementById('groupModalMovieSearchContainer');
                ...
                timeoutId = setTimeout(() => {
                    fetch(`/movies/search?q=${encodeURIComponent(query)}`, {
                        ...
                        .then(movies => {
                            ...
                            if (movies.length === 0) {
                                resultsDiv.innerHTML =
                                    '<div class="p-3 text-gray-500">No movies found</div>';
                            } else {
                                movies.slice(0, 5).forEach(movie => {
                                    ...
                                    const safeYear = movie.release_date ? new Date(

<div id="create-group-music-review-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50" style="display: none;">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-2xl shadow-xl max-w-6xl w-full min-h-[400px]">
            ...
            <div class="p-8">
                <form id="create-group-music-review-form"
                    action="{{ route('groups.reviews.store', ['group' => $group->id, 'type' => 'music']) }}"
                    method="POST">
                    @csrf
                    <input type="hidden" name="type" value="music">
                    <input type="hidden" name="group_id" value="{{ $group->id }}">

                    <x-ui.media-search type="music" searchId="groupModalMusicSearch"
                        searchResultsId="groupModalMusicSearchResults" selectedId="groupModalSelectedMusic"
                        inputName="spotify_id" removeBtnId="groupRemoveMusicBtn"
                        searchPlaceholder="Search for an album or song..." label="What music did you listen to?" />
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex gap-2" id="group-music-star-rating">
                            @for ($i = 1; $i <= 5; $i++)
                                <button type="button"
                                    class="group-music-star-btn text-3xl text-gray-300 hover:text-yellow-400 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:rounded-full transition-colors"
                                    data-rating="{{ $i }}">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="content" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#38157a]"
                            rows="5" placeholder="Share your thoughts..."></textarea>
                    </div>

                    <div class="flex justify-end space-x-3">
                        <x-ui.button type="button" id="cancel-group-music-review-button"
                ...
                timeoutId = setTimeout(() => {
                    fetch(`/music/search?q=${encodeURIComponent(query)}`, {
                        ...
                        .then(songs => {
                            ...
                            if (songs.length === 0) {
                                resultsDiv.innerHTML =
                                    '<div class="p-3 text-gray-500">No music found</div>';
                            } else {
                                songs.slice(0, 5).forEach(song => {
                                    const div = document.createElement('div');
                                    div.className =
                                        'p-3 hover:bg-gray-100 cursor-pointer border-b flex items-center transition-colors';
                                ...
                                <div class="flex items-center w-full">
                                    ...
                                    div.addEventListener('click', () => {
                                        ...
                                        selectedTitle.textContent = song
                                            .title;
                                        selectedArtist.textContent = song
                                            .creator;
                                        selectedYear.textContent = song
                                            .releaseyear || '';
                                        selectedCover.src = song
                                            .coverimage || '';
